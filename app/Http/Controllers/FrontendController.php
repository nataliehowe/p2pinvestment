...
class FrontendController extends Controller
{
    ...
    public function pifiAbout()
{       
    $data['random'] = '';
    return view($this->theme . 'p2p-general-pages/about', $data);
}

public function pifiFinancialReports()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/financial-reports', $data);
}

public function pifiLoans()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/loans', $data);
}

public function pifiSave()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/pifi-save', $data);
}

public function pifiBlog()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/blog', $data);
}

public function pifiInvesting()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/investing', $data);
}

public function pifiMarket()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/market', $data);
}

public function pifiReferAFriend()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/refer-a-friend', $data);
}

public function pifiContactUs()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/contact-us', $data);
}

public function pifiInvestWithPifi()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/invest-with-pifi', $data);
}

public function pifiOriginatorsPartnership()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/originators-partnership', $data);
}

public function pifiStatistics()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/statistics', $data);
}

public function pifiFaq()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/faq', $data);
}

public function pifiLoanOriginators()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/loan-originators', $data);
}

public function pifiClubs()
{       
    $data['your_data'] = '';
    return view($this->theme . 'p2p-general-pages/pifi-clubs', $data);
}

public function pifiThankYou()
{       
    $data['your_data'] = '';
    return view($this->theme . 'verify/thank-you', $data);
}

public function pifiVerificationError()
{       
    $data['your_data'] = '';
    return view($this->theme . 'verify/verification-error', $data);
}

public function pifiUser()
{       
    $data['your_data'] = '';
    return view($this->theme . 'user/dashboard', $data);
}

public function verifyPifiEmail($token)
{
    $userPifi = UserPifi::where('verification_token', $token)->first();

    if ($userPifi) {
        // Mark the user as verified
        $userPifi->update(['verified' => true]);
        $userPifi->update(['status' => true]);

        // Redirect to the "thank-you" route with query parameters
        return redirect()->route('thank-you', ['email' => $userPifi->email]);
    } else {
        // Handle invalid or expired tokens
        // Redirect to the "error" route with query parameters
        return redirect()->route('verification-error', ['email' => $userPifi->email]);
    }
}
