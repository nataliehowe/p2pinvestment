(function(global, factory) {
    if (typeof exports === "object" && typeof module !== "undefined") {
        module.exports = factory(require("lodash-4"), require("tb.xerrors"), require("jf.utils"), require("jf.api"), require("tbjson.ajv2tb"), require("tbjson.deepmerge"), require("tb.translate"));
    } else if (typeof define === "function" && define.amd) {
        define([ "lodash-4", "tb.xerrors", "jf.utils", "jf.api", "tbjson.ajv2tb", "tbjson.deepmerge", "tb.translate" ], function() {
    ...
    function populateField(domEl, elSchema, initValue, jfTinyInstance) {
        ...
        if (nodeName === "select") {
            ...
            ASSERT.isArray(elSchema.enum, {
                msg: "Found select field, but missing enum in schema, please check your schema first near $schema$.",
                msgParams: {
                    schema: elSchema
                }
            });
            ...
            for (var i = 0, l = elSchema.enum.length; i < l; i++) {
                ...
                var selected = initValue === undefined ? false : initValue instanceof Array ? initValue.indexOf(value) >= 0 : initValue === value;
    ...
    function populateLabel(elSel, elSchema) {
        ...
        el = getEl(el);
        ASSERT(el, {
            msg: "Cannot find <label> in DOM for $schema$ `title`. Looking for DOM with selector $elsel$, but $el$ found.",
            msgParams: {
                schema: elSchema,
                elsel: elSel,
                el: el
    ...
    function populateDescr(elSel, elSchema) {
        ...
        el = getEl(el);
        ASSERT(el, {
            msg: "Cannot find <label> in DOM for $schema$ `description`. Looking for DOM with selector $elsel$, but $el$ found.",
            msgParams: {
                schema: elSchema,
                elsel: elSel,
                el: el
    ...
    function jfTiny(method, opts) {
        ...
        ASSERT.isNotNil(apiUrl, {
            msg: "Missing apiUrl. Please check if you`ve given `apiUrl` property to jfTiny`s constructor OR set it globally through `TB.API_URL`."
        });
        ASSERT.isNotNil(apiUrl, {
            msg: "Missing apiKey. Please check if you`ve given `apiKey` property to jfTiny`s constructor OR set it globally through `TB.API_KEY`."
        });
        this.s = opts;
        ...
        ASSERT(this.s.lockFields === "false" || /disabled|readonly/.test(this.s.lockFields), {
            msg: 'lockFields property must be false, "disabled"(default) or "readonly", but given $value$',
            msgParams: {
                value: this.s.lockFields
            }
        });
    ...
    p.init = function init(s) {
        ...
        if (s.formDesc !== undefined) {
            ...
            this.api.formLoad(s.payloadParams).then(function(result) {
                TRACE("Result from server: ", result);
                ASSERT(result.schema, {
                    msg: "No schema given from the server!",
                    code: "JFT600"
                });
                ASSERT(result.content, {
                    msg: "No content given from the server!",
                    code: "JFT650"
                });
    ...
    p._loopByIds = function _loopByIds(fieldIds, cb) {
        ASSERT.isArray(fieldIds, {
            code: 10510,
            msg: "fieldIds must be an array, but $fieldIds$ given",
            msgParams: {
                fieldIds: fieldIds
            }
        });
        for (var i = 0, l = fieldIds.length; i < l; i++) {
            ...
            ASSERT(field, {
                code: 10520,
                msg: "Unable to find field with id `$fieldId$`",
                msgParams: {
                    fieldId: fieldId
                }
            });
    ...
    p.populateForm = function populateForm() {
        ...
        for (var i = 0, l = fields.length; i < l; i++) {
            var field = fields[i];
            ASSERT(field.id, {
                msg: "Missing field HTML attribute id on field. Please check the additional data of this error for more info.",
                msgParams: {
                    dom: field
                }
            });
            ...
            ASSERT(_.isPlainObject(elSchema), {
                code: 10010,
                msg: "Schema must be an object, but $schema$ given. I guess there is an field in DOM which resolves to path with $path$, so please check whether your DOM has loaded the most recent HTML ids and other meta data.",
                msgParams: {
                    schema: elSchema,
                    path: jsonPath,
                    value: value,
    ...
    p.submit = function submit() {
            ...
            if (el.type === "file") {
                ...
                if ((el.getAttribute("tb-jf-type") === "fileRequest" || schemaElement.format === "file") && el.files.length > 0) {
                    ...
                    if (el.files.length > 0) {
                        (function(jsonPath) {
                            var filePromise = TB.File.loadFile(el.files[0], "DataURL").then(function(fileContent) {
                ...
                if (passwordFields[jsonPath].hasOwnProperty("confirm") && passwordFields[jsonPath]["confirm"] !== "" && passwordFields[jsonPath].hasOwnProperty("old") && passwordFields[jsonPath]["old"] !== "" && passwordFields[jsonPath].hasOwnProperty("new") && passwordFields[jsonPath]["new"] !== "") {
        ...
        Promise.all(filePromises).then(function(values) {
            ...
            return self.api.formSubmit(_.merge({
                content: _.merge(self.content, result)
            }, self.s.payloadParams), files).then(function(result) {
                self.content = result.content;
                self.toggleLockFields(els, false);
                self.populateForm();
                self.triggerEvent("submitted", {
                    instance: self
                });
            });
        }).catch(function(err) {
    ...
    p.clearErrors = function clearErrors(elSel) {
        ...
        ASSERT(el, {
            code: 10530,
            msg: "Unable to find field $elSel$",
            msgParams: {
                elSel: elSel
            }
        });
        ...
        ASSERT(errMsg, {
            code: 10540,
            msg: "Unable to find msg for field $elSel$",
            msgParams: {
                elSel: elSel
            }
        });
    ...
    p.setErrors = function setErrors(errList) {
        for (var i = 0, l = errList.length; i < l; i++) {
            ...
            ASSERT(errMsg, {
                msg: "Unable to find the element to set message in. Please check your DOM, because no element with id selector $id$ found.",
                msgParams: {
                    id: replaceFieldId(id, "msg")
                }
            });
