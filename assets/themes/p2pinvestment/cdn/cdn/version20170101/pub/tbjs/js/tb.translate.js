!function(a) {
    ...
    b.format = function(a, f) {
        ...
        for (h = 0; o > h; h++) if (p = c(a[h]), "string" === p) q[q.length] = a[h]; else if ("array" === p) {
            ...
            e.json.test(j[8]) ? q[q.length] = g : (!e.number.test(j[8]) || r && !j[3] ? s = "" : (s = r ? "+" : "-", 
            g = g.toString().replace(e.sign, "")), l = j[4] ? "0" === j[4] ? "0" : j[4].charAt(1) : " ", 
            m = j[6] - (s + g).length, k = j[6] && m > 0 ? d(l, m) : "", q[q.length] = j[5] ? s + g + k : "0" === l ? s + k + g : k + s + g);
        }
        return q.join("");
...
(function(global, factory) {
    ...
    var fakeIntlNumberFormat = function(locale, config) {
        ...
        this.config = config || {};
        this.format = function(value) {
            var maximumFractionDigits = typeof this.config.maximumFractionDigits === "number" ? this.config.maximumFractionDigits : this.config.style === "currency" ? 2 : undefined;
    ...
    Translate.prototype = {
        _init: function(translations, locales, module) {
            ASSERT(_.isArray(locales) && !_.isEmpty(locales) > 0, {
                code: "TBJS/I18N/2001",
                msg: "Bad locales list"
            });
            this.module = module;
        ...
        getdate: function(date) {
            if (!_.isDate(date) || _.isString(date) && date.length === 0) {
        ...
        gethash: function(fallback, hash) {
            ...
            if (_.isEmpty(hash)) {
                ...
                var string = localeString.length ? localeString : fallback;
